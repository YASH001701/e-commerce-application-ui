import React, { useState } from 'react'
import { Link } from 'react-router-dom'
import SearchIcon from '@mui/icons-material/Search';
import MoreVertIcon from '@mui/icons-material/MoreVert';

const Header = () => {
  let [isOpen,setIsOpen]=useState(false)
  return (
    
    <header className="grid grid-cols-2 p-2 ">
    <div className='flex'>
      
     
      <Link to={"/"}><img src="https://static-assets-web.flixcart.com/batman-returns/batman-returns/p/images/fkheaderlogo_exploreplus-44005d.svg"
       width="160" height="40" title="Flipkart"/></Link>
      
        <div className="flex item-center bg-black-500 p-2 w-full ml-20" >
        <SearchIcon className='w-5 h-5 ml-2  text-black' /> 
        <input className='bg-blue-50 ml-3 flex text-gray-900 text-xl rounded-lg block w-full'
         placeholder='Search for Products, Brands and More'  required/>
         </div>
         </div>
        <div  className='flex item-center justify-between mx-20'>
          <div className='relative' onMouseEnter={()=> setIsOpen(true)} onMouseLeave={()=> setIsOpen(false)}>
<div className=' w-20 h-25 flex item-center justify-center text-lg hover:text-white hover:bg-blue-500 p-3 hover:rounded-xl'>
<Link to={"/login"}>Login</Link>
</div>

          {isOpen &&(<div>
            <ul>
              <li className='mb-2'>
                <Link className='flex item-center justify-between border-b-2 p-2' title='Sign Up'>
                <div>New Customer?</div>  </Link>
                <div>
                 <Link className='text-blue 500 font-bold' title='Sign Up'>Sign Up</Link>
                </div>
              </li>
              <li className='mb-2 hover:bg-gray-100 p-1' title='My Profile'>
               <Link>My Profile</Link>
              </li>
              <li className='mb-2 hover:bg-gray-100 p-1' title='Flipkart Plus Zone'>
               <Link>Flipkart Plus Zone</Link>
              </li>
              <li className='mb-2 hover:bg-gray-100 p-1' title='Order'>
               <Link>Order</Link>
              </li>
              <li className='mb-2 hover:bg-gray-100 p-1' title='WishList'>
               <Link>WishList</Link>
              </li>
              <li className='mb-2 hover:bg-gray-100 p-1' title='Rewards'>
               <Link>Reward</Link>
              </li>
              <li className='mb-2 hover:bg-gray-100 p-1' title='GiftCard'>
               <Link>Giftcard</Link>
              </li>

              
            </ul>
            </div>
          
          
          )}
   </div> 
          
        <div className='h-15 w-6 '>
          
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path d="M0 24C0 10.7 10.7 0 24 0H69.5c22 0 41.5 12.8 50.6 32h411c26.3 0 45.5 25 38.6 50.4l-41 152.3c-8.5 31.4-37 53.3-69.5 53.3H170.7l5.4 28.5c2.2 11.3 12.1 19.5 23.6 19.5H488c13.3 0 24 10.7 24 24s-10.7 24-24 24H199.7c-34.6 0-64.3-24.6-70.7-58.5L77.4 54.5c-.7-3.8-4-6.5-7.9-6.5H24C10.7 48 0 37.3 0 24zM128 464a48 48 0 1 1 96 0 48 48 0 1 1 -96 0zm336-48a48 48 0 1 1 0 96 48 48 0 1 1 0-96z"/></svg>
          </div>
          <Link className='mx-30  ' to={"/home"}> Cart</Link>
          <div className='h-5 w-6 '>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path d="M547.6 103.8L490.3 13.1C485.2 5 476.1 0 466.4 0H109.6C99.9 0 90.8 5 85.7 13.1L28.3 103.8c-29.6 46.8-3.4 111.9 51.9 119.4c4 .5 8.1 .8 12.1 .8c26.1 0 49.3-11.4 65.2-29c15.9 17.6 39.1 29 65.2 29c26.1 0 49.3-11.4 65.2-29c15.9 17.6 39.1 29 65.2 29c26.2 0 49.3-11.4 65.2-29c16 17.6 39.1 29 65.2 29c4.1 0 8.1-.3 12.1-.8c55.5-7.4 81.8-72.5 52.1-119.4zM499.7 254.9l-.1 0c-5.3 .7-10.7 1.1-16.2 1.1c-12.4 0-24.3-1.9-35.4-5.3V384H128V250.6c-11.2 3.5-23.2 5.4-35.6 5.4c-5.5 0-11-.4-16.3-1.1l-.1 0c-4.1-.6-8.1-1.3-12-2.3V384v64c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V384 252.6c-4 1-8 1.8-12.3 2.3z"/></svg>
          </div>
          <h1 className='mx-30 '  >
          <Link  to={"/becomeaSeller"}>  Become a Seller</Link>
          </h1>
        <MoreVertIcon className='h-5 w-6'/>

          </div>
             
        
      
  
  
   
  </header>
  )
}

export default Header;



